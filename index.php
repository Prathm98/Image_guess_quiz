    ...
    <script type="text/javascript">
      $(document).ready(function() {
        $('#login').click(function(){
          ...
          if(flag == 0){
            $.post('assests/_inc/login_check.php',{usr:uname,pass:upass},function(data){
              if(data == 1){
                window.location.href = "admin/";
              }else if(data == 2){
                window.location.href = "event/";
              }else if(data == -1){
                $('.err').html('<div style="color:red; font-size:12px">Either Username or Password is wrong.</div>');    
              }else{
                $('.err').html('<div style="color:red; font-size:12px">Something went wrong.</div>');    
              }
            });
          }else{
      ...
      <div class="col s10 m6 l4">
        <div class="card z-depth-3">
          <div class="card-content">
            ...
            <div style="text-align: right;"><a class="waves-effect waves-light btn" id="login">login<i class="material-icons right">chevron_right</i></a></div>
            <div style="padding: 10px; text-align: center; padding-bottom: 20px">
        <div class="divider"></div>
        <h6 style="text-align: center; color: gray">OR</h6>
        <a class="waves-effect waves-light btn" href="reg.php">Register<i class="material-icons right">create</i></a>
      </div>
          </div>
        </div>
      </div>
